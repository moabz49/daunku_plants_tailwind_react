...
const Hero = () => {
  ...
  return (
    // <div h-screen relative flex flex-col items-center> → top div always flex col
    <section className='h-screen relative flex flex-col items-center '
    // Background image not using tailwind but the style built in functionality in JScript. Its more reliable.
        style={{
    ...
    >
        <div className='relative w-full max-w-[1490px] flex items-center justify-between pt-10 mx-auto px-10'>
            ...
            <ul className='hidden md:flex items-center gap-10 lg:gap-[68px]'>
                {/* Create One Nav Link component passing props that trigger certain styles and functionality --> genius */}
            ...
            {/* block:  it ensures that the element takes up the full width of its container, and each element is displayed on a new line */}
            <HiMenuAlt3 
                size={30}
            ...
            /> 
            <div
                // the menubar is block and will only be displayed when showMobileMenu is ? true
                className={`block md:hidden w-full fixed ${!showMobileMenu ? "-top-[410px]" : "top-0"} left-0 bg-[#dde0e5] h-[410px] transition-all duration-[800ms] shadow-xl z-10 py-8 px-12 rounded-b-xl`}
            >
                <AiOutlineClose 
                    size={25}
                ...
                {/* mobileMenu prop will apply styles to the NavLinks that are exclusive to the tradition desktop display. */}
                <NavLink to="services" mobileMenu> Services</NavLink>
                <NavLink to="products" mobileMenu> Shop</NavLink>
                <NavLink to="reference" mobileMenu> References</NavLink>
        ...
        {/* Re-usable FADEIn component that we is a <div> which applies basic but affective animations, pass props to determin DIRECTION of 
        animation and other props for styles  */}
    ...
    {/* This on the bottom of the image like add shadow effect to the bottom of the image  */}
    {/* Absolute so overlaps the relative flow. ---> Genius */}
